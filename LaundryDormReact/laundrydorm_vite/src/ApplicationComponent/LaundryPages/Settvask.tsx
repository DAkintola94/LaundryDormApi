...
import { globalFetchData, profileProps } from "@/lib/authCall";
...
export const Settvask = () => {
  ...
  const handleSubmit = async (e:React.FormEvent<HTMLFormElement>) => {
    ...
    const laundrySessionData:sessionProps = { //variable name on the left, that gets value from the right, needs to match our variable model name in c sharp
    ...
    try{
      const methodCallbackResponse = await setLaundryCall(API_BASE_URL, laundrySessionData)
      if(!methodCallbackResponse) return setPending(false);

      //Else
        navigate('/success', {
        replace: true, //prevent user from going back
        state: {
          ID: methodCallbackResponse, //Value we want to pass to the redirected page
        }
    });
        ...
        <form onSubmit={handleSubmit} className="relative z-10 w-full px-4 sm:px-6 lg:px-8">
          <div className="mx-auto w-full max-w-3xl"> {/* increased max width to make form bigger */}
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-lg shadow-lg p-8 md:p-12"> {/* larger padding and card */}
             ...
             rounded py-3 px-4 mb-4 text-lg leading-tight focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-300" id="grid-first-name" value={dataFromApi.userName || ""} readOnly />
                  ...
                  <input className="appearance-none block w-full bg-gray-100 text-gray-900 border border-gray-300 
             rounded py-3 px-4 mb-4 text-lg leading-tight focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-300" id="grid-first-name" value={dataFromApi.email || ""} readOnly />
             ...
             { errorMsg && 
                <span className="text-red-600 mb-4"> {errorMsg} </span>
                  ...
                  <select id="laundryTime" className="w-full bg-white placeholder:text-slate-400 text-black text-base border border-slate-200 rounded pl-3 pr-8 py-3 
                  transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer"
                  ...
                  <select id="machineId" className="w-full bg-white placeholder:text-black text-slate-700 text-base border border-slate-200 rounded pl-3 pr-8 py-3 
                  transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer"
